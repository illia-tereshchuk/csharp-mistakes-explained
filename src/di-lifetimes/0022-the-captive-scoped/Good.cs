...
// Exhibit #0022: the fix

using Microsoft.Extensions.DependencyInjection;

var services = new ServiceCollection();
services.AddScoped<CurrentUser>();    // one per request
services.AddScoped<CartService>();    // matches its dependency's lifetime - one per request too

// Validate scopes: the container will now REFUSE a singleton that eats a scoped service.
...
class CartService(CurrentUser user)
{
